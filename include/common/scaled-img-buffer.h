 ...
 *
 * If drop_img is set true, lab_img_destroy() is called when the node is
 * destroyed or the image is updated.
 */
struct scaled_img_buffer *scaled_img_buffer_create(struct wlr_scene_tree *parent,
	struct lab_img *img, int width, int height, int padding, bool drop_img);
