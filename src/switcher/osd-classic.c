...
struct switcher_osd_classic_item {
	struct switcher_osd_item base;
		...
		const float *bg_color, int field_widths_sum, int x, int y)
{
	...
	struct switcher_osd_field *field;
	wl_list_for_each(field, &rc.window_switcher.fields, link) {
		...
		if (field->content == LAB_FIELD_ICON) {
			...
			switcher_osd_field_get_content(field, &buf, view);
...
switcher_osd_classic_create(struct output *output, struct wl_array *views)
{
	...
	lab_scene_rect_create(output->switcher_osd.tree, &bg_opts);
	...
	if (show_workspace) {
		...
		struct scaled_font_buffer *font_buffer =
			scaled_font_buffer_create(output->switcher_osd.tree);
	...
	wl_array_for_each(view, views) {
		struct switcher_osd_classic_item *item = znew(*item);
		wl_list_append(&output->switcher_osd.items, &item->base.link);
		item->base.view = *view;
		item->base.tree = wlr_scene_tree_create(output->switcher_osd.tree);
		node_descriptor_create(&item->base.tree->node,
			LAB_NODE_SWITCHER_OSD_ITEM, NULL, item);
		/*
		 *    OSD border
		 * +---------------------------------+
		 * |                                 |
...
error:;
	/* Center OSD */
	wlr_scene_node_set_position(&output->switcher_osd.tree->node,
...
struct switcher_osd_impl switcher_osd_classic_impl = {
	.create = switcher_osd_classic_create,
	.update = switcher_osd_classic_update,
};
