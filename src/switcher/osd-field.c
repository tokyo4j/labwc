...
static const struct field_converter field_converter[LAB_FIELD_COUNT] = {
	...
	[LAB_FIELD_CUSTOM]             = { '\0', switcher_osd_field_set_custom },
};

void
switcher_osd_field_set_custom(struct buf *buf, struct view *view, const char *format)
{
	if (!format) {
...
void
switcher_osd_field_arg_from_xml_node(struct switcher_osd_field *field,
...
bool
switcher_osd_field_is_valid(struct switcher_osd_field *field)
{
...
void
switcher_osd_field_get_content(struct switcher_osd_field *field,
...
void
switcher_osd_field_free(struct switcher_osd_field *field)
{
	zfree(field->format);
	zfree(field);
}
