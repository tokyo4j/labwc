...
struct switcher_osd_thumbnail_item {
		...
		const float *text_color, const float *bg_color, int y)
{
	...
	switcher_osd_field_set_custom(&buf, view,
...
static struct switcher_osd_thumbnail_item *
		...
		struct output *output)
{
	...
	struct switcher_osd_thumbnail_item *item = znew(*item);
	wl_list_append(&output->switcher_osd.items, &item->base.link);
	struct wlr_scene_tree *tree = wlr_scene_tree_create(parent);
	node_descriptor_create(&tree->node, LAB_NODE_SWITCHER_OSD_ITEM, NULL, item);
	item->base.tree = tree;
	item->base.view = view;
...
switcher_osd_thumbnail_create(struct output *output, struct wl_array *views)
{
	...
	wlr_scene_node_set_position(&output->switcher_osd.tree->node, lx, ly);
}
...
struct switcher_osd_impl switcher_osd_thumbnail_impl = {
	.create = switcher_osd_thumbnail_create,
	.update = switcher_osd_thumbnail_update,
};
