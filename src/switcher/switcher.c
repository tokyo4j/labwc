...
destroy_switcher(struct server *server)
{
	...
	wl_list_for_each_safe(view, tmp, &server->switcher.views, switcher_link) {
		wl_list_remove(&view->switcher_link);
		view->switcher_link = (struct wl_list){0};
	}
}
...
static struct view *
get_next_cycle_view(struct server *server, enum lab_cycle_dir dir)
{
	struct switcher_state *switcher = &server->switcher;
	assert(switcher->cycle_view);

	struct wl_list *link;
	if (dir == LAB_CYCLE_DIR_FORWARD) {
		link = switcher->cycle_view->switcher_link.next;
		if (link == &server->switcher.views) {
			link = link->next;
		}
	} else {
		link = switcher->cycle_view->switcher_link.prev;
		if (link == &server->switcher.views) {
			link = link->prev;
		}
	}
	struct view *view = wl_container_of(link, view, switcher_link);
	return view;
}
...
switcher_on_view_destroy(struct view *view)
{
	...
	if (switcher->cycle_view == view) {
		...
		switcher->cycle_view = get_next_cycle_view(server,
...
switcher_begin(struct server *server, enum lab_cycle_dir direction)
{
	...
	/*
	 * Initially select the next/previous window from the currently focused
	 * (or topmost) one
	 */
	struct switcher_state *switcher = &server->switcher;
	struct view *view;
	wl_list_for_each(view, &switcher->views, switcher_link) {
		if (view == server->active_view) {
			switcher->cycle_view = view;
			break;
		}
	}
	if (!switcher->cycle_view) {
		switcher->cycle_view = wl_container_of(switcher->views.next,
			switcher->cycle_view, switcher_link);
	}
	switcher->cycle_view = get_next_cycle_view(server, direction);
...
switcher_cycle(struct server *server, enum lab_cycle_dir direction)
{
	...
	server->switcher.cycle_view = get_next_cycle_view(server, direction);
	update_switcher(server);
}
...
init_switcher(struct server *server)
{
	...
	for_each_view(view, &server->views, rc.window_switcher.criteria) {
		wl_list_append(&server->switcher.views, &view->switcher_link);
	}
	if (wl_list_length(&server->switcher.views) <= 0) {
