...
theme_builtin(struct theme *theme, struct server *server)
{
	...
	theme->osd_window_switcher_list.width = 600;
	theme->osd_window_switcher_list.width_is_percent = false;
	theme->osd_window_switcher_list.padding = 4;
	theme->osd_window_switcher_list.item_padding_x = 10;
	theme->osd_window_switcher_list.item_padding_y = 1;
	theme->osd_window_switcher_list.item_active_border_width = 2;
	theme->osd_window_switcher_list.item_icon_size = -1;
...
entry(struct theme *theme, const char *key, const char *value)
{
	...
	struct window_switcher_list_theme *switcher_listl_theme =
		&theme->osd_window_switcher_list;
	...
	if (match_glob(key, "osd.window-switcher.style-list.width")
			|| match_glob(key, "osd.window-switcher.width")) {
		if (strrchr(value, '%')) {
			switcher_list_theme->width_is_percent = true;
		} else {
			switcher_list_theme->width_is_percent = false;
		}
		switcher_list_theme->width = get_int_if_positive(
			value, "osd.window-switcher.style-list.width");
	}
	if (match_glob(key, "osd.window-switcher.style-list.padding")
			|| match_glob(key, "osd.window-switcher.padding")) {
		switcher_list_theme->padding = get_int_if_positive(
			value, "osd.window-switcher.style-list.padding");
	}
	if (match_glob(key, "osd.window-switcher.style-list.item.padding.x")
			|| match_glob(key, "osd.window-switcher.item.padding.x")) {
		switcher_list_theme->item_padding_x =
			get_int_if_positive(
				value, "osd.window-switcher.style-list.item.padding.x");
	}
	if (match_glob(key, "osd.window-switcher.style-list.item.padding.y")
			|| match_glob(key, "osd.window-switcher.item.padding.y")) {
		switcher_list_theme->item_padding_y =
			get_int_if_positive(
				value, "osd.window-switcher.style-list.item.padding.y");
	}
	if (match_glob(key, "osd.window-switcher.style-list.item.active.border.width")
			|| match_glob(key, "osd.window-switcher.item.active.border.width")) {
		switcher_list_theme->item_active_border_width =
			get_int_if_positive(
				value, "osd.window-switcher.style-list.item.active.border.width");
	}
	if (match_glob(key, "osd.window-switcher.style-list.item.icon.size")
			|| match_glob(key, "osd.window-switcher.item.icon.size")) {
		switcher_list_theme->item_icon_size =
			get_int_if_positive(
				value, "osd.window-switcher.style-list.item.icon.size");
	}
...
{
	struct window_switcher_list_theme *switcher_list_theme =
		&theme->osd_window_switcher_list;
	...
	if (switcher_list_theme->item_icon_size <= 0) {
		switcher_list_theme->item_icon_size = osd_font_height;
	}
	int osd_field_height =
		MAX(osd_font_height, switcher_list_theme->item_icon_size);
	switcher_list_theme->item_height = osd_field_height
		+ 2 * switcher_list_theme->item_padding_y
		+ 2 * switcher_list_theme->item_active_border_width;
	...
	if (switcher_list_theme->width_is_percent) {
		switcher_list_theme->width =
			MIN(switcher_list_theme->width, 100);
	}
