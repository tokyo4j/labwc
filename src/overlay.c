...
static bool
adjacent_output_exists_from_cursor(struct seat *seat, struct output *output,
		enum wlr_direction direction)
{
	return wlr_output_layout_adjacent_output(
		seat->server->output_layout, direction,
		output->wlr_output, seat->cursor->x, seat->cursor->y);
}

static void
show_edge_overlay(struct seat *seat, struct edge_snap_info info)
{
	if (!rc.snap_overlay_enabled) {
		return;
	}
	if (seat->overlay.active.tiled_state == info.tiled_state
			&& seat->overlay.active.output == info.output) {
		return;
	}
	inactivate_overlay(&seat->overlay);
	seat->overlay.active.tiled_state = info.tiled_state;
	seat->overlay.active.output = info.output;

	int delay;
	if (adjacent_output_exists_from_cursor(seat, info.output, info.primary_direction)) {
	...
	if (delay > 0) {
		...
		/* Show overlay now */
		struct wlr_box box = get_edge_snap_box(info.tiled_state, info.output);
		show_overlay(seat, &seat->overlay.edge_rect, &box);
	}
}
...
overlay_update(struct seat *seat)
{
	...
	struct edge_snap_info info = get_edge_snap_info(seat);
	if (info.tiled_state != VIEW_TILED_NONE) {
		show_edge_overlay(seat, info);
		return;
	}
