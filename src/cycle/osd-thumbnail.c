...
cycle_osd_thumbnail_create(struct output *output)
{
	...
	output->cycle_osd.items_tree = wlr_scene_tree_create(output->cycle_osd.tree);
	...
	wl_list_for_each(view, &server->cycle.views, cycle_link) {
		struct cycle_osd_thumbnail_item *item = create_item_scene(
			output->cycle_osd.items_tree, view, output);
		if (!item) {
			break;
		}
	...
	cycle_osd_scroll_init(output, padding + items_width - SCROLLBAR_W, padding,
...
{
	cycle_osd_scroll_update(output);
