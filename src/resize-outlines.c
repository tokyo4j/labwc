...
resize_outlines_update(struct view *view, struct wlr_box new_geo)
{
	...
	if (!outlines->rect) {
		float *colors[3] = {
			view->server->theme->resize_outlines_color[0],
			view->server->theme->resize_outlines_color[1],
			view->server->theme->resize_outlines_color[2],
		};
		int width = view->server->theme->resize_outlines_thickness;
...
{
	struct resize_outlines *outlines = &view->resize_outlines;
	if (!outlines->rect || !outlines->rect->tree->node.enabled) {
		return;
	}
