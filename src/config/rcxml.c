...
{
	struct switcher_osd_field *field = znew(*field);
	wl_list_append(&rc.window_switcher.fields, &field->link);

	xmlNode *child;
	char *key, *content;
	LAB_XML_FOR_EACH(node, child, key, content) {
		switcher_osd_field_arg_from_xml_node(field, key, content);
	}
}
...
entry(xmlNode *node, char *nodename, char *content)
{
	 ...
	 */
		...
		if (!strcasecmp(content, "classic")) {
			rc.window_switcher.style = SWITCHER_OSD_STYLE_CLASSIC;
		} else if (!strcasecmp(content, "thumbnail")) {
			rc.window_switcher.style = SWITCHER_OSD_STYLE_THUMBNAIL;
		} else {
		...
		if (!strcasecmp(content, "all")) {
			rc.window_switcher.output_criteria = SWITCHER_OSD_OUTPUT_ALL;
		} else if (!strcasecmp(content, "pointer")) {
			rc.window_switcher.output_criteria = SWITCHER_OSD_OUTPUT_POINTER;
		} else if (!strcasecmp(content, "keyboard")) {
			rc.window_switcher.output_criteria = SWITCHER_OSD_OUTPUT_KEYBOARD;
		} else {
...
validate(void)
{
	...
	wl_list_for_each_safe(field, field_tmp, &rc.window_switcher.fields, link) {
		...
		if (!switcher_osd_field_is_valid(field) || field_width_sum > 100) {
