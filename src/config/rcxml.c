...
fill_action_query(char *nodename, char *content, struct action *action, struct parser_state *state)
{
	...
	if (!strcasecmp(nodename, "identifier")) {
		...
		state->current_view_query->tiled = parse_tiled_edge(content);
...
post_processing(void)
{
	...
	if (rc.snap_edge_corner_range < rc.snap_edge_range) {
		wlr_log(WLR_ERROR, "<snapping><cornerRange> must be bigger"
			" than <snapping><range>");
		rc.snap_edge_corner_range = rc.snap_edge_range;
	}
}
