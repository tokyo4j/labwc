...
tiled_edge_to_view_edge(enum tiled_edge tiled_edge)
{
	switch (tiled_edge) {
	case TILED_TOP:
		return VIEW_EDGE_UP;
...
static enum tiled_edge
tiled_edge_invert(enum tiled_edge tiled_edge)
{
	switch (tiled_edge) {
	case TILED_TOP:
		return TILED_BOTTOM;
			...
			bool across_outputs, bool store_natural_geometry)
{
	...
	enum view_edge four_way_edge = tiled_edge_to_view_edge(edge);

	if (across_outputs && view->tiled == edge
			&& four_way_edge != VIEW_EDGE_INVALID
			&& view->maximized == VIEW_AXIS_NONE) {
		/* We are already tiled for this edge; try to switch outputs */
		output = output_get_adjacent(view->output, four_way_edge,
						/* wrap */ false);
