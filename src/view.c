...
view_toggle_always_on_top(struct view *view)
{
	...
	wl_signal_emit_mutable(&view->events.set_always_on_top, NULL);
}
...
view_toggle_visible_on_all_workspaces(struct view *view)
{
	...
	ssd_update_geometry(view->ssd);
	wl_signal_emit_mutable(&view->events.set_sticky, NULL);
}
...
view_set_shade(struct view *view, bool shaded)
{
	...
	wl_signal_emit_mutable(&view->events.set_shaded, NULL);
}
...
view_init(struct view *view)
{
	...
	wl_signal_init(&view->events.set_always_on_top);
	wl_signal_init(&view->events.set_sticky);
	wl_signal_init(&view->events.set_shaded);
...
view_destroy(struct view *view)
{
	...
	assert(wl_list_empty(&view->events.set_always_on_top.listener_list));
	assert(wl_list_empty(&view->events.set_sticky.listener_list));
	assert(wl_list_empty(&view->events.set_shaded.listener_list));
