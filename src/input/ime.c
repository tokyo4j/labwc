		...
		struct wl_resource *resource, uint32_t id)
{
	...
	wl_signal_emit_mutable(&wlr_manager_v3->events.new_text_input, text_input);
}
...
handle_new_text_input(struct wl_listener *listener, void *data)
{
	...
	if (!is_v1 && relay->seat->wlr_seat != wlr_text_input->seat) {
		return;
	}
...
input_method_relay_create(struct seat *seat)
{
	...
	text_input_manager_v1_init(server.wl_display, server.text_input_manager);
