...
struct osd_list_scene_item {
	struct view *view;
	struct wlr_scene_node *highlight_outline;
};

static void
osd_list_create(struct output *output, struct wl_array *views)
{
	assert(!output->osd_scene.tree);

	struct server *server = output->server;
	struct theme *theme = server->theme;
	struct window_switcher_list_theme *t =
		&theme->osd_window_switcher_list;
	bool show_workspace = wl_list_length(&rc.workspace_config.workspaces) > 1;
	const char *workspace_name = server->workspaces.current->name;

	int w = t->width;
	if (t->width_is_percent) {
		w = output->wlr_output->width * t->width / 100;
	}
	int h = wl_array_len(views) * t->item_height
		+ 2 * rc.theme->osd_border_width + 2 * t->padding;
	if (show_workspace) {
		/* workspace indicator */
		h += t->item_height;
	}
	...
	int y = theme->osd_border_width + t->padding;
	...
	if (show_workspace) {
		...
		wlr_scene_node_set_position(&font_buffer->scene_buffer->node,
			x, y + (t->item_height - font_height(&font)) / 2);
	...
	int field_widths_sum = w - 2 * theme->osd_border_width
		- 2 * t->padding - 2 * t->item_active_border_width
		- (nr_fields + 1) * t->item_padding_x;
	...
	wl_array_for_each(view, views) {
		struct osd_list_scene_item *item =
		 ...
		 * +---------------------------------+
		 */
		int x = theme->osd_border_width + t->padding
			+ t->item_active_border_width + t->item_padding_x;
		...
		wl_list_for_each(field, &rc.window_switcher.fields, link) {
			...
			if (field->content == LAB_FIELD_ICON) {
				int icon_size = MIN(field_width, t->item_icon_size);
			...
			if (node) {
				int item_height = t->item_height;
				wlr_scene_node_set_position(node,
					x, y + (item_height - height) / 2);
			}
			x += field_width + t->item_padding_x;
		}
		...
		struct lab_scene_rect_options highlight_opts = {
			...
			.width = w - 2 * theme->osd_border_width - 2 * t->padding,
...
static void
osd_list_update(struct output *output)
{
	struct osd_list_scene_item *item;
...
struct osd_impl osd_list_impl = {
	.create = osd_list_create,
	.update = osd_list_update,
};
