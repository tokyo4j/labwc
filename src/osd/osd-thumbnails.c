		...
		struct output *output)
{
	...
	if (thumb_buffer) {
		...
		struct wlr_box thumb_bounds = {
			.x = thumb_item_padding,
			.y = thumb_item_padding,
			.width = thumb_item_width - 2 * thumb_item_padding,
			.height = title_y - 2 * thumb_item_padding,
		};
	...
	/* Icon */
	struct scaled_icon_buffer *icon_buffer = NULL;
	const char *app_id = view_get_string_prop(view, "app_id");
	if (app_id) {
		icon_buffer = scaled_icon_buffer_create(tree, server,
