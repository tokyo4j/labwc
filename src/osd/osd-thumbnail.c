...
create_title(struct wlr_scene_tree *parent,
		struct window_switcher_thumbnail_theme *t, const char *title,
		const float *title_color, const float *bg_color, int y)
{
	struct scaled_font_buffer *buffer =
		scaled_font_buffer_create(parent);
	scaled_font_buffer_update(buffer, title,
		t->item_width - 2 * t->item_padding,
		&rc.font_osd, title_color, bg_color);
	wlr_scene_node_set_position(&buffer->scene_buffer->node,
		(t->item_width - buffer->width) / 2, y);
	return buffer;
}
		...
		struct output *output)
{
	...
	int padding = theme->border_width + t->item_padding;
	int title_y = t->item_height - padding - t->title_height;
	struct wlr_box thumb_bounds = {
		.x = padding,
		.y = padding,
		.width = t->item_width - 2 * padding,
	...
	struct lab_scene_rect_options opts = {
		...
		.bg_color = t->item_active_bg_color,
		.nr_borders = 1,
		.border_colors = (float *[1]) { t->item_active_border_color },
		.border_width = t->item_active_border_width,
	};
	...
	if (title) {
		item->normal_title = create_title(item->tree, t, title,
			theme->osd_label_text_color,
			theme->osd_bg_color, title_y);
		item->active_title = create_title(item->tree, t, title,
			theme->osd_label_text_color,
			t->item_active_bg_color, title_y);
	}

	/* icon */
	struct scaled_icon_buffer *icon_buffer = scaled_icon_buffer_create(
		item->tree, server, t->item_icon_size, t->item_icon_size);
	scaled_icon_buffer_set_view(icon_buffer, view);
	int x = (t->item_width - t->item_icon_size) / 2;
	int y = title_y - padding - t->item_icon_size + 10; /* slide by 10px */
		...
		int nr_thumbs, int *nr_rows, int *nr_cols)
{
	...
	int max_bg_width = t->max_width;
	if (t->max_width_is_percent) {
		max_bg_width = output_width * t->max_width / 100;
	}
	...
	*nr_cols = nr_thumbs;
	while (1) {
		...
		int bg_width = *nr_cols * t->item_width
			+ theme->osd_border_width + t->padding;
...
osd_thumbnail_create(struct output *output, struct wl_array *views)
{
	...
	struct window_switcher_thumbnail_theme *t =
		&theme->osd_window_switcher_thumbnail;
	int padding = theme->osd_border_width + t->padding;
	...
	wl_array_for_each(view, views) {
		...
		int x = (index % nr_cols) * t->item_width + padding;
		int y = (index / nr_cols) * t->item_height + padding;
	...
	struct lab_scene_rect_options bg_opts = {
		.width = nr_cols * t->item_width + 2 * padding,
		.height = nr_rows * t->item_height + 2 * padding,
