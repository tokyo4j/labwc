...
struct osd_item {
	struct view *view;
	struct wlr_scene_node *highlight_node;
};

static void update_osd(struct server *server);

static void
destroy_osd_scene(struct output *output)
{
...
create_osd_scene(struct output *output, struct wl_array *views)
{
	...
	struct scaled_rect_buffer *bg_buffer = scaled_rect_buffer_create(
		output->osd_scene.tree, w, h, theme->osd_border_width,
		bg_color, theme->osd_border_color);
	assert(bg_buffer);
	...
	if (show_workspace) {
		...
		int indicator_width = font_width(&font, workspace_name);
		indicator_width = MIN(indicator_width, w - 2 * theme->osd_border_width);
		/* Center workspace indicator on the x axis */
		int x = (w - indicator_width) / 2;
		...
		scaled_font_buffer_update(font_buffer, workspace_name, indicator_width,
	...
	wl_array_for_each(view, views) {
		...
		struct scaled_rect_buffer *highlight_buffer =
			scaled_rect_buffer_create(output->osd_scene.tree,
				highlight_w, highlight_h, border_width,
				transparent, text_color);
		assert(highlight_buffer);
		item->highlight_node = &highlight_buffer->scene_buffer->node;
		wlr_scene_node_set_position(item->highlight_node,
			theme->osd_border_width + theme->osd_window_switcher_padding, y);
		wlr_scene_node_set_enabled(item->highlight_node, false);
...
update_item_highlight(struct output *output)
{
	...
	wl_array_for_each(item, &output->osd_scene.items) {
		wlr_scene_node_set_enabled(item->highlight_node,
